...
## Варианты реализации [*алгоритма Дейкстры*](#алгоритм-дейкстры)
...
# Фибоначчиева куча (формальное описание)
...
```
<strong id="merge">Слияние с другой кучей</strong> <br>
...
Для доказательства времени работы фибоначчиевой кучи воспользуемся амортизированным анализом методом потенциалов. 
...
<strong id="insert_math">Вставка элемента</strong> <br>
...
<strong id="merge_math">Слияние двух куч</strong> <br>
...
<strong id="delete_min_math">Удаление минимума</strong> <br>

Перед оценкой амортизировнной сложности данной операции докажем несколько лемм:

<strong id="lemma_1">Лемма 1</strong> <br/>
Для всех целых чисел n >= 2 выполняется слудующее равенство:
$$F_n=1+\sum_{i=1}^{n-2}F_i,    $$
где $F_n$ n-ое число Фибоначчи.

___Доказательство___ <br/>
...
<strong id="lemma_2">Лемма 2</strong> <br/>
...
<strong id="lemma_3">Лемма 3</strong> <br/>
...
___Переход:___
$$F_n=F_{n-1}+F_{n-2}=\frac{φ^{n-1}-(-φ)^{1-n}}{\sqrt{5}}+\frac{φ^{n-2}-(-φ)^{2-n}}{\sqrt{5}}=$$
$$=\frac{1}{\sqrt{5}}(φ^{n-1}-(-φ)^{1-n}+φ^{n-2}-(-φ)^{2-n})=$$
...
<strong id="lemma_4">Лемма 4</strong> <br/>
...
Тогда соглассно [*лемме 2*](#lemma_2), в дереве, корнем которого является вершина $v$ содержится не менее $F_k$ вершин, что в свою очередь по [*лемме 3*](#lemma_3) равно $O(φ^k)$, то есть $\ge φ^k$.
...
Согласно [*лемме 4*](#lemma_4) $O(degree)=O(log(n))$

<strong id="decrease-key_math">Уменьшение элемента</strong> <br>
